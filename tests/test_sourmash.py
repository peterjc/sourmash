...
def test_compare_containment_abund_flatten(c):
    ...
    assert (
        "NOTE: --containment means signature abundances are flattened"
...
def test_compare_containment_require_scaled(c):
    ...
    assert "must use scaled signatures with --containment option" in c.last_result.err
    assert c.last_result.status != 0
...
def test_plot_override_labeltext():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["plot", "cmp", "--labeltext", "new.labels.txt"],
            in_directory=location,
        )
...
def test_do_sourmash_sbt_search_output():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["index", "zzz", "short.fa.sig", "short2.fa.sig", "-k", "31"],
            in_directory=location,
        )
...
def test_do_sourmash_sbt_search_check_bug():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["index", "zzz", testdata1, testdata2, "-k", "31"],
            in_directory=location,
        )
...
def test_do_sourmash_index_bad_args():
    with utils.TempDirectory() as location:
        ...
        print(out, err)
        assert (
            "cannot specify more than one of --dna/--rna/--protein/--hp/--dayhoff"
            in err
        )
        assert status != 0
...
def test_do_sourmash_index_traverse():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash", ["index", "-k", "31", "zzz", "."], in_directory=location
        )
...
def test_do_sourmash_index_sparseness():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["index", "-k", "31", "zzz.sbt.json", ".", "--sparseness", "1.0"],
            in_directory=location,
        )
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["search", "short.fa.sig", "zzz.sbt.json"],
            in_directory=location,
        )
        print(out)
...
def test_do_sourmash_sbt_combine():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["sbt_combine", "joined", "zzz.sbt.zip", "zzz.sbt.zip"],
            in_directory=location,
        )
...
def test_do_sourmash_sbt_search_bestonly_scaled():
    ...
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["compute", testdata1, testdata2, "--scaled", "1"],
            in_directory=location,
        )
...
def test_gather_f_match_orig(c):
    ...
    c.run_sourmash("gather", testdata_combined, "-o", "out.csv", *testdata_sigs)
...
def test_multigather_metagenome_query_with_lca(c):
    ...
    cmd = "multigather --query {} --db 47+63.sbt.zip -k 31 --threshold-bp=0".format(
        lca_db
    )
    cmd = cmd.split(" ")
    c.run_sourmash(*cmd)
...
def test_multigather_metagenome_query_from_file_with_addl_query(c):
    ...
    # second gather query
    assert (
        "4.9 Mbp      100.0%  100.0%    NC_003198.1 Salmonella enterica subsp..." in out
    )
    assert "found 1 matches total;" in out
    assert "the recovered matches hit 100.0% of the query" in out
...
def test_gather_abund_10_1(c):
    ...
    status, out, err = c.run_sourmash("gather", query, "-o", "xxx.csv", *against_list)
...
def test_gather_output_unassigned_with_abundance(c):
    ...
    c.run_sourmash(
        "gather", query, against, "--output-unassigned", c.output("unassigned.sig")
    )
...
def test_storage_convert():
    ...
    with utils.TempDirectory() as location:
        ...
        args = [
            ...
            "convert",
            "-b",
            """'ZipStorage("{}")'""".format(os.path.join(location, "v2.sbt.zip")),
            testsbt,
        ]
        status, out, err = utils.runscript("sourmash", args, in_directory=location)
