...
def test_dna_override_1():
    factory = _signatures_for_sketch_factory(["k=21,scaled=2000,abund"], "dna", False)
    params_list = list(factory.get_compute_params())
...
def test_dna_override_bad_1():
    with pytest.raises(ValueError):
        factory = _signatures_for_sketch_factory(
            ["k=21,scaledFOO=2000,abund"], "dna", False
        )


def test_dna_override_bad_2():
    with pytest.raises(ValueError):
        factory = _signatures_for_sketch_factory(["k=21,protein"], "dna", False)
...
def test_protein_override_bad_2():
    with pytest.raises(ValueError):
        factory = _signatures_for_sketch_factory(["k=21,dna"], "protein", False)
...
def test_dayhoff_override_bad_2():
    with pytest.raises(ValueError):
        factory = _signatures_for_sketch_factory(["k=21,dna"], "dayhoff", False)
...
def test_hp_override_bad_2():
    with pytest.raises(ValueError):
        factory = _signatures_for_sketch_factory(["k=21,dna"], "hp", False)
...
def test_do_sourmash_sketchdna():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash", ["sketch", "dna", testdata1], in_directory=location
        )
...
def test_do_sourmash_sketchdna_output_valid_file():
    ...
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["sketch", "dna", "-o", sigfile, testdata1, testdata2, testdata3],
...
def test_do_sourmash_sketchdna_output_stdout_valid():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["sketch", "dna", "-o", "-", testdata1, testdata2, testdata3],
            in_directory=location,
        )
...
def test_do_sourmash_sketchdna_singleton():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["sketch", "dna", "--singleton", testdata1],
            in_directory=location,
        )
...
def test_do_sourmash_sketchdna_name():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["sketch", "dna", "--merge", "foo", testdata1, "-o", "foo.sig"],
            in_directory=location,
        )
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["sketch", "dna", "--name", "foo", testdata1, "-o", "foo2.sig"],
            in_directory=location,
        )
...
def test_do_sourmash_sketchdna_name_fail_no_output():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["sketch", "dna", "--merge", "foo", testdata1],
            in_directory=location,
            fail_ok=True,
        )
        assert status == -1
...
def test_do_sourmash_sketchdna_name_from_first():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["sketch", "dna", "--name-from-first", testdata1],
            in_directory=location,
        )
...
def test_do_sourmash_sketchdna_multik():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["sketch", "dna", "-p", "k=31,k=21", testdata1],
            in_directory=location,
        )
...
def test_do_sketch_dna_override_protein_fail():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["sketch", "dna", "-p", "k=7,num=500,protein", testdata1],
            in_directory=location,
            fail_ok=True,
        )
...
def test_do_sketch_protein_override_dna_fail():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["sketch", "protein", "-p", "k=7,num=500,dna", testdata1],
            in_directory=location,
            fail_ok=True,
        )
...
def test_do_sketch_translate_multik_with_protein():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["sketch", "translate", "-p", "k=7,k=10,num=500", testdata1],
            in_directory=location,
        )
        outfile = os.path.join(location, "short.fa.sig")
        assert os.path.exists(outfile)
...
def test_do_sketch_translate_multik_with_hp():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["sketch", "translate", "-p", "k=7,k=10,num=500", "--hp", testdata1],
            in_directory=location,
        )
        outfile = os.path.join(location, "short.fa.sig")
        assert os.path.exists(outfile)
...
def test_do_sketch_protein_multik_input():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["sketch", "protein", "-p", "k=7,k=10,num=500", testdata1],
            in_directory=location,
        )
...
def test_do_sourmash_sketchdna_multik_outfile():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["sketch", "dna", "-p", "k=21,k=31", testdata1, "-o", outfile],
            in_directory=location,
        )
        assert os.path.exists(outfile)
...
def test_do_sourmash_check_protein_comparisons():
    ...
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["sketch", "protein", "-p", "k=7,num=500", "--singleton", testdata1],
            in_directory=location,
        )
