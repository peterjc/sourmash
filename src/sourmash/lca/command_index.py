            ...
            if lineage:
                ...
                if ident in assignments:
                    if assignments[ident] != tuple(lineage):
                        if not force:
                            raise Exception(
                                "multiple lineages for identifier {}".format(ident)
                            )
                else:
...
def index(args):
    ...
    notify(
        "Building LCA database with ksize={} scaled={} moltype={}.",
        args.ksize,
        args.scaled,
        moltype,
    )
    ...
    for filename in inp_files:
        ...
        for sig in it:
            ...
            notify(
                "\r... loading signature {} ({} of {}); skipped {} so far",
                str(sig)[:30],
                n,
                total_n,
                n_skipped,
    ...
    if n_skipped:
        notify(
            "... loaded {} signatures; skipped {} because of --require-taxonomy.",
            total_n,
            n_skipped,
        )
    else:
    ...
    if record_duplicates or record_no_lineage or record_remnants or unused_lineages:
        ...
        if record_remnants:
            notify(
                "WARNING: no signatures for {} spreadsheet rows.", len(record_remnants)
            )
        if unused_lineages:
