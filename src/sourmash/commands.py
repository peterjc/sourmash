...
def compare(args):
    ...
    if args.containment:
        ...
        similarity = compare_all_pairs(
            siglist, args.ignore_abundance, n_jobs=args.processes
        )
...
def search(args):
    ...
    databases = sourmash_args.load_dbs_and_sigs(
        args.databases, query, not args.containment
    )
...
def categorize(args):
    ...
    for queryfile, query, query_moltype, query_ksize in loader:
        ...
        if results:
            ...
            notify("for {}, found: {:.2f} {}", query, best_hit_sim, best_hit_query)
            best_hit_query_name = best_hit_query.name
        else:
            notify("for {}, no match found", query)

        if csv_w:
            csv_w.writerow([queryfile, query, best_hit_query_name, best_hit_sim])
...
def gather(args):
    ...
    if args.cache_size == 0:
        cache_size = None
    databases = sourmash_args.load_dbs_and_sigs(
        args.databases, query, False, cache_size=cache_size
    )
    ...
    if args.num_results and len(found) == args.num_results:
        print_results("(truncated gather because --num-results={})", args.num_results)
